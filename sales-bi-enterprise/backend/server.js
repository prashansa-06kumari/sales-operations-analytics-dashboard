...
function registerRoutes() {
  ...
  // Datasets
  apiRouter.post('/datasets/upload', upload.single('file'), (req, res) => {
  ...
  // Analytics
  apiRouter.get('/analytics/kpis', (req, res) => {
  ...
  apiRouter.get('/analytics/category-sales', (req, res) => {
  ...
  apiRouter.get('/analytics/forecast', (req, res) => {
    db.all(`SELECT SUBSTR(order_date, 1, 7) as month, SUM(revenue) as revenue FROM sales GROUP BY month ORDER BY month`, (err, rows) => {
      ...
      for (let i = 1; i <= 6; i++) {
        ...
        forecast.push({
          month: monthStr,
          revenue: Math.round(forecastVal),
          isForecast: true,
        });
      }
  ...
  apiRouter.get('/analytics/scatter-data', (req, res) => {
    db.all(`SELECT marketing_spend as x, revenue as y, quantity as z FROM sales`, (err, rows) => {
      res.json(rows || []);
    });
  });

  apiRouter.get('/analytics/operational-trend', (req, res) => {
    db.all(
      `SELECT SUBSTR(order_date, 1, 10) as day, SUM(CASE WHEN delivery_days <= 5 THEN 1 ELSE 0 END) as onTime, SUM(CASE WHEN delivery_days > 5 THEN 1 ELSE 0 END) as delayed FROM sales GROUP BY day ORDER BY day DESC LIMIT 7`,
      (err, rows) => {
  ...
  apiRouter.get('/analytics/operational-metrics', (req, res) => {
    db.get(
      `SELECT AVG(delivery_days) as avgDeliveryDays, COUNT(*) as totalShipments, SUM(CASE WHEN delivery_days > 5 THEN 1 ELSE 0 END) as delayedOrders FROM sales`,
      (err, row) => {
  ...
  apiRouter.get('/reports/sales-data', (req, res) => {
    db.all(
      `SELECT s.order_id, s.order_date, s.revenue, s.profit, p.name as product_name, r.name as region_name, c.name as customer_name FROM sales s JOIN products p ON s.product_id = p.id JOIN regions r ON s.region_id = r.id JOIN customers c ON s.customer_id = c.id ORDER BY s.order_date DESC LIMIT 100`,
      (err, rows) => {
        res.json(rows || []);
      }
    );
  ...
  // Alias for clients whose base URL omits /api (e.g. VITE_API_URL=https://host.onrender.com)
  app.use(apiRouter);
}
